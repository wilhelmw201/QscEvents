//本文件填写2PlayerSelectEvent_0Entry选项7086d0ee-47db-4aa7-aefd-9d54343fc9db在事件执行周期内的相关阶段函数
...
using Qsc;
using System.Linq;
public class EventOption_7086d0ee47db4aa7aefd9d54343fc9db : Event_41f1ee150d194fd4a8c2dff6f90ce379
{
#endif
    ...
    private bool OnVisibleCheck()
    {
        ...
        if (Event.OptionDescrs.Length <= thisId) return false;
        if (Event.chances[thisId] >= 100) return true;
        if (QscCoreUtils.RandInt32(this.TaiwuEvent, 0, 100) <= Event.chances[thisId])
        {
            return true;
        }
        return false;
    ...
    private string OnGetReplacedContent()
    {
        int thisId = 4;
        PlayerSelectEvent Event = (PlayerSelectEvent)QscCoreUtils.EventList.Last();
        if (Event.OptionDescrs.Length <= thisId) return "";

        return Event.OptionDescrs[thisId];
    }
    ...
    private string OnSelect()
    {
        ...
        int thisId = 4;
        PlayerSelectEvent Event = (PlayerSelectEvent)QscCoreUtils.EventList.Last();
        return QscCoreUtils.CallEvent(Event.Dests[thisId]);
    }
