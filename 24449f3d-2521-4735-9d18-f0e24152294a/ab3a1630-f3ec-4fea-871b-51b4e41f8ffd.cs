//本文件填写SimpleLootEvent_2PickJump选项ab3a1630-f3ec-4fea-871b-51b4e41f8ffd在事件执行周期内的相关阶段函数
...
public class EventOption_ab3a1630f3ec4fea871b51b4e41f8ffd : Event_24449f3d252147359d18f0e24152294a
{
#endif
    ...
    private string OnSelect()
    {
        ...
        Event.current += 1;
        if (ArgBox.Get("QscLSel", out ItemKey item))
        {
            EventHelper.TransferInventoryItem(EventHelper.GetCharacterById(Event.containerId), DomainManager.Taiwu.GetTaiwu(), item);
        }
        else
        {
            DomainManager.Taiwu.GmCmd_AddResource(GameData.Domains.DomainManager.TaiwuEvent.MainThreadDataContext, 6, 
                Event.goldPerCount * (Event.count - Event.current + 1));
            return Qsc.QscCoreUtils.PopAndReturn(false);
        }
        if (Event.current < Event.count)
        {
            return ("538db91b-072b-4b0b-943d-c0ea9a40be6e");
        }
        else
        {
            return Qsc.QscCoreUtils.PopAndReturn(false);
