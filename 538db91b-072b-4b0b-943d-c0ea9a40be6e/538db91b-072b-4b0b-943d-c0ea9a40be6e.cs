...
#if IN_IDE
public class Event_538db91b072b4b0b943dc0ea9a40be6e : TaiwuEventItem
{
#endif
    ...
    public override void OnEventEnter()
    {
        ...
        EventHelper.Log("pick event got char " + charid + " with " + LootChar.GetInventory().Items.Count + " items");

        ArgBox.Set("SimpleLootPick", Event.count - Event.current);
        ArgBox.Set("SimpleLootGold", Event.goldPerCount);
        ArgBox.Remove<ItemKey>("QscLSel");
        var Filter = new SelectItemFilter();
        Filter.FilterTemplateId = Config.ItemFilterRules.DefKey.SelectGiftGrade0;
        Filter.Key = "QscLSel";
        DomainManager.Taiwu.GetTaiwu().SetInventory(DomainManager.Taiwu.GetTaiwu().GetInventory(), GameData.Domains.DomainManager.TaiwuEvent.MainThreadDataContext);
        LootChar.SetInventory(LootChar.GetInventory(), GameData.Domains.DomainManager.TaiwuEvent.MainThreadDataContext);
        EventHelper.SelectCharacterItemRequest(Event.containerId, ArgBox, Filter);
