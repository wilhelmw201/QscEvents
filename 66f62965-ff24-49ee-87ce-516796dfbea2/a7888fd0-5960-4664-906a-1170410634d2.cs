//本文件填写SimpleSkillPick_1Pick选项a7888fd0-5960-4664-906a-1170410634d2在事件执行周期内的相关阶段函数
...
public class EventOption_a7888fd059604664906a1170410634d2 : Event_66f62965ff2449ee87ce516796dfbea2
{
#endif
    ...
    private bool OnVisibleCheck()
    {
        ...
        int skillid = -1;
        bool isGF = false;
        bool res = ArgBox.Get("SimpleSkillPickOpt" + thisChoice, ref skillid);
        if (!res || skillid < 0)
        {
            //AdaptableLog.Info("SimpleSkillPick: Option 0 is not pickable!");
            return false;
    ...
    private string OnGetReplacedContent()
    {
        ...
        bool isGF = false;
        bool res = ArgBox.Get("SimpleSkillPickOpt" + thisChoice, ref skillid);
        bool res1 = ArgBox.Get("SimpleSkillPickGF" + thisChoice, ref isGF);

        if (isGF)
        {
            return result + "习得：" + Config.CombatSkill.Instance.GetItem((short)skillid).Name;
        }
        else
            return result + "习得：" + Config.LifeSkill.Instance.GetItem((short)skillid).Name;
    }
    ...
    private string OnSelect()
    {
        ...
        if (!(ev is EditorSimpleSkillPickEvent))
        {
            throw new System.InvalidOperationException("Popped incorrect item from event stack, got" + ev.GetType());
        }
        EditorSimpleSkillPickEvent Event = (EditorSimpleSkillPickEvent)ev;
        Event.current += 1;
        ...
        bool res = ArgBox.Get("SimpleSkillPickOpt" + thisChoice, ref skillid);
        bool res1 = ArgBox.Get("SimpleSkillPickGF" + thisChoice, ref isGF);
        ArgBox.Set("SimpleSkillPickOpt" + thisChoice, -1);
        ArgBox.Remove<int>("SimpleSkillPickOpt" + thisChoice);
        ArgBox.Remove<bool>("SimpleSkillPickGF" + thisChoice);
        if (isGF)
        {
            QscLootUtil.ActivateCombatSkill((short)skillid);
        }
        else
        {
            QscLootUtil.ActivateLifeSkill((short)skillid);

        }



        return "73e70c39-85a9-430d-a607-3514509670ae";
    }
